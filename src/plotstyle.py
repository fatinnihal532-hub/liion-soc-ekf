...
def _round_coords(svg_text, places=1):
    ...
    "372.48" draws exactly the same line. At the width these figures render,
    one decimal place is a tenth of a pixel, so nothing is lost that any
    screen or printer could resolve.
...
def step_points(t, y):
    ...
    reproduces exactly the same picture - the line between two adjacent
    samples is the same near-vertical edge it always was - from a few hundred
    points instead.

    This is lossless for a signal that only ever holds a level, which is what
    a pole voltage is. It is not decimation: no sample is dropped that the
    eye or the geometry could tell apart. Nothing that varies smoothly, such
    as a load current, should be passed through it.

    The reason it matters: GitHub stops rendering an SVG in a README somewhere
    above 128 KB, and a figure nobody can see is worth nothing.
    """
    import numpy as np
